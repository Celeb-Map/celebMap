...
export default function CoursePlanner({ restaurant, map, maps, onClose }: {
  ...
  useEffect(() => {
    ...
    const overlays = markedPlaces.map(place => {
      ...
      button.type = 'button';
      button.className = `flex h-10 w-10 items-center justify-center rounded-full border-2 border-white text-sm font-black shadow-lg ${place.candidateId === activeId ? 'bg-neon-400 text-plum-900' : 'bg-plum-700 text-white'}`;
      ...
      if (place.candidateId === activeId) {
        ...
        label.className = 'mb-1 max-w-44 truncate rounded-xl bg-plum-700 px-3 py-1.5 text-xs font-bold text-white shadow-lg';
  ...
  return (
    <section aria-labelledby="course-planner-title" className={`absolute bottom-0 left-0 right-0 z-30 flex flex-col overflow-hidden rounded-t-[28px] border border-plum-100 bg-white shadow-[0_-12px_40px_rgba(60,26,71,0.18)] ${collapsed ? 'h-24' : 'h-[62%]'}`}>
      <div className="flex shrink-0 items-center gap-3 px-5 py-4">
        ...
        <div className="min-w-0 flex-1"><h2 ref={titleRef} tabIndex={-1} id="course-planner-title" className="text-base font-black text-plum-900 outline-none">{stops.length ? '나의 여행 코스' : '오늘은 어디까지 가볼까요?'}</h2><p className="mt-0.5 truncate text-xs text-plum-500">{stops.length ? `${stops.length}곳 · 구간 직선거리 합계 ${distanceLabel(totalDistance)}` : `${restaurant.name}에서 출발해요`}</p></div>
      ...
      {!collapsed && <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pb-5">
        ...
        {stops.length > 0 && <>
          ...
          <p className="mb-3 text-[11px] leading-5 text-plum-500">영업시간은 방문 전 확인해 주세요. 거리는 직선거리이며, 실제 길찾기와 이동 시간은 아직 제공하지 않아요.</p>
          <ol className="space-y-2">{stops.map((place, index) => <li key={place.candidateId}>
            {index > 0 && <p className="mb-2 ml-4 border-l-2 border-dashed border-plum-200 py-1 pl-5 text-[10px] text-plum-400">직선 {distanceLabel(straightDistance(stops[index - 1], place))}</p>}
          ...
          <p role="status" className="mt-3 text-[11px] text-plum-500">{stops.length}곳 · 순서를 바꾸면 지도 번호와 직선거리도 바뀌어요.</p>
