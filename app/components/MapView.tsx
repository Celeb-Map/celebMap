...
export default function MapView({ restaurant }: { restaurant: Restaurant | null }) {
  ...
  const [message, setMessage] = useState(t("맛집을 선택하면 주변 여행 정보를 보여드려요."));
  const [mapReady, setMapReady] = useState(false);
  const [mapError, setMapError] = useState<string | null>(() => appKey ? null : t("NEXT_PUBLIC_KAKAO_MAP_KEY가 설정되지 않았습니다."));

  const loadNearbyPlaces = useCallback(async (position: Coordinates, signal: AbortSignal) => {
    setStatus('loading');
    setMessage(t("주변 관광지와 숙박을 불러오고 있어요."));
    try {
      const params = new URLSearchParams({ latitude: position.latitude.toString(), longitude: position.longitude.toString(), lang: locale });
      const response = await fetch(`/api/tourism/nearby?${params}`, { signal });
      const data = await response.json() as NearbyResponse;
      if (signal.aborted) return;
      if (!response.ok) throw new Error(t("관광 정보를 불러오지 못했습니다."));
      setPlaces(data.places);
      setStatus('success');
      const accommodations = data.places.filter(place => place.kind === 'accommodation').length;
      setMessage(data.places.length ? t('반경 3km 내 관광지와 숙박 {count}곳 · 숙박 {stays}곳', { count: data.places.length, stays: accommodations }) : t("반경 3km 내 등록된 장소가 없어요."));
    } catch (error) {
      if (signal.aborted) return;
      setStatus('error');
      setMessage(error instanceof Error ? error.message : t("관광 정보를 불러오지 못했습니다."));
    }
  }, [locale, t]);
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      void loadNearbyPlaces(coordinates, controller.signal);
    }, 0);
  ...
  useEffect(() => {
    ...
    loadKakaoMap(appKey).then(maps => {
      ...
      restaurantLabel.textContent = restaurant?.name ?? t("선택한 맛집");
      ...
      restaurantMarker.textContent = t("맛집");
      locationNode.append(restaurantLabel, restaurantMarker);
      locationNode.setAttribute('aria-label', t('{name} 기준 위치', { name: restaurant?.name ?? t('선택한 맛집') }));
      ...
      if (!cancelled) {
        setMapReady(false);
        setMapError(error instanceof Error ? error.message : t("Kakao 지도를 표시하지 못했습니다."));
      }
    });
    return () => { cancelled = true; };
  }, [appKey, coordinates, restaurant, t]);
  ...
  useEffect(() => {
    ...
    placeOverlaysRef.current = visiblePlaces.map(place => {
      ...
      marker.setAttribute('aria-label', `${place.title} ${isAccommodation ? t("숙박") : t("관광지")}`);
  ...
  }, [activePlaceId, mapReady, placeFilter, places, courseOpen, t]);
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    const query = new URLSearchParams({ contentTypeId: selectedPlace.contentTypeId, lang: locale });
    fetch(`/api/tourism/detail/${selectedPlace.id}?${query}`, { signal: controller.signal })
      .then(async response => {
        const data = await response.json() as DetailResponse;
        if (controller.signal.aborted) return;
        if (!response.ok || !data.detail) throw new Error(data.message ?? t("상세 정보를 불러오지 못했습니다."));
    ...
    return (
      <div className="flex h-[calc(100dvh-116px)] items-center justify-center bg-plum-50 px-8 text-center">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-plum-500">{t("홈이나 검색에서 맛집을 누르면")}<br />{t("주변 관광지와 숙박을 지도에서 보여드려요.")}</p>
          <p className="mt-4 rounded-full bg-white px-4 py-2 text-xs font-bold text-plum-400 shadow-sm">{t("현재 위치 권한은 사용하지 않아요")}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="relative flex h-[calc(100dvh-116px)] flex-col">
      ...
      {!courseOpen && <div className={`absolute bottom-0 left-0 right-0 z-20 overflow-hidden rounded-t-[28px] border border-b-0 border-plum-100 bg-white/95 shadow-[0_-12px_40px_rgba(60,26,71,0.14)] backdrop-blur-xl transition-[height] duration-300 ${panelOpen ? 'h-80' : 'h-[88px]'}`}>
        <button type="button" onClick={() => setPanelOpen(open => !open)} aria-expanded={panelOpen} aria-controls="nearby-places-panel" className="flex h-[88px] w-full items-center gap-3 px-5 text-left">
          ...
          <span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className="text-sm font-extrabold text-plum-900">{t("맛집 주변 여행 장소")}</span><span className="rounded-full bg-neon-400/20 px-2 py-0.5 text-[11px] font-extrabold text-plum-700">{places.length}{t("곳")}</span></span><span className="mt-1 block truncate text-xs text-plum-400">{panelOpen ? `${t("관광")} ${attractionCount} · ${t("숙박")} ${accommodationCount}` : t("관광지와 숙박 목록 보기")}</span></span>
        ...
        {panelOpen && (
          <div id="nearby-places-panel" className="h-[calc(100%-88px)] overflow-y-auto overscroll-contain border-t border-plum-100/80 px-4 pb-5 no-scrollbar">
            <div className="sticky top-0 z-10 -mx-4 mb-3 flex gap-2 border-b border-plum-100 bg-white px-4 pb-3 pt-3 shadow-[0_6px_12px_rgba(60,26,71,0.04)]">
              {([
                ['all', t("전체"), places.length],
                ['attraction', t("관광"), attractionCount],
                ['accommodation', t("숙박"), accommodationCount],
            ...
            {filteredPlaces.length === 0 ? <div className="flex h-28 flex-col items-center justify-center text-center">{placeFilter === 'accommodation' ? <BedDouble size={24} className="mb-2 text-orange-300" /> : <MapPin size={24} className="mb-2 text-plum-300" />}<p className="text-sm font-semibold text-plum-500">{status === 'success' ? t("반경 3km 내 {kind} 없어요.", { kind: placeFilter === "accommodation" ? t("숙박이") : t("장소가") }) : t("위치를 확인하면 주변 장소가 표시돼요.")}</p></div> : filteredPlaces.map(place => (
              <button type="button" key={place.id} onClick={() => focusPlace(place)} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left transition-all ${activePlaceId === place.id ? 'border-neon-400 bg-neon-50 shadow-sm' : 'border-transparent bg-plum-50/70 hover:border-plum-200'}`}>
                ...
                <div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><span className={`shrink-0 rounded px-1.5 py-0.5 text-[9px] font-black ${place.kind === 'accommodation' ? 'bg-orange-100 text-orange-700' : 'bg-plum-100 text-plum-600'}`}>{place.kind === 'accommodation' ? t("숙박") : t("관광")}</span><p className="truncate text-sm font-bold text-plum-900">{place.title}</p></div><p className="mt-0.5 truncate text-xs text-plum-400">{place.address || t("주소 정보 없음")}</p></div>
                <span className="shrink-0 rounded-full bg-white px-2.5 py-1 text-[11px] font-extrabold text-plum-700 shadow-sm">{t(formatDistance(place.distanceMeters))}</span>
              </button>
            ))}
            </div>
          </div>
      ...
      {selectedPlace && (
        <div className="absolute inset-0 z-40 flex items-end" role="dialog" aria-modal="true" aria-labelledby="place-detail-title">
          <button type="button" className="absolute inset-0 bg-plum-900/35 backdrop-blur-[2px]" onClick={closeDetail} aria-label={t("상세 정보 닫기")} />
          <section className="relative z-10 max-h-[78%] w-full overflow-y-auto rounded-t-[30px] bg-white shadow-[0_-18px_50px_rgba(29,8,36,0.24)] no-scrollbar">
            ...
            <button type="button" onClick={closeDetail} aria-label={t("상세 정보 닫기")} className="absolute right-4 top-4 z-20 flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-plum-700 shadow-md"><X size={18} /></button>
            ...
            <div className="relative h-48 bg-plum-100">
              ...
              <span className={`absolute bottom-3 left-4 rounded-full px-3 py-1 text-[11px] font-black text-white shadow-sm ${selectedPlace.kind === 'accommodation' ? 'bg-orange-500' : 'bg-plum-700'}`}>{selectedPlace.kind === 'accommodation' ? t("숙박") : t("관광지")}</span>
            </div>
            ...
            <div className="px-5 pb-7 pt-5">
              <div className="pr-6">
                ...
                <div className="mt-2 flex items-start gap-1.5 text-xs leading-5 text-plum-500"><MapPin size={14} className="mt-0.5 shrink-0" /><span>{placeDetail?.address || selectedPlace.address || t("주소 정보 없음")}</span></div>
              </div>

              <div className="mt-4 flex gap-2">
                <span className="rounded-full bg-plum-50 px-3 py-1.5 text-xs font-extrabold text-plum-700">{t("현재 기준")}{t(formatDistance(selectedPlace.distanceMeters))}</span>
                {placeDetail?.zipcode && <span className="rounded-full bg-plum-50 px-3 py-1.5 text-xs font-bold text-plum-500">{t("우편번호")}{placeDetail.zipcode}</span>}
              </div>

              {detailStatus === 'loading' && <div className="mt-5 flex items-center gap-2 rounded-2xl bg-plum-50 p-4 text-xs font-semibold text-plum-500"><LoaderCircle size={16} className="animate-spin" />{t("상세 정보를 불러오고 있어요.")}</div>}
              {detailStatus === 'error' && <div className="mt-5 rounded-2xl bg-plum-50 p-4 text-xs leading-5 text-plum-500">{t("추가 상세 정보는 제공되지 않아요. 기본 위치 정보는 확인할 수 있습니다.")}</div>}
              {placeDetail?.overview && <div className="mt-5"><h3 className="text-sm font-extrabold text-plum-900">{t("장소 소개")}</h3><p className="mt-2 whitespace-pre-line text-sm leading-6 text-plum-600">{placeDetail.overview}</p></div>}
              ...
              <a href={`https://map.kakao.com/link/map/${encodeURIComponent(selectedPlace.title)},${selectedPlace.latitude},${selectedPlace.longitude}`} target="_blank" rel="noreferrer" className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-plum-700 text-sm font-extrabold text-neon-400 shadow-sm"><ExternalLink size={16} />{t("카카오맵에서 보기")}</a>
            </div>
          </section>
        </div>
      )}
